...
namespace VehicleSimulator
{
    ...
    public partial class ucGauge : Control
    {
        ...
        System.ComponentModel.Category("ucGauge"),
                ...
                if (BackgroundImage != null)
                {
                    switch (BackgroundImageLayout)
                    {
                        ...
                        case ImageLayout.Zoom:
                            if (Height != 0
                                && BackgroundImage.Width / (Single)Width < BackgroundImage.Height / (Single)Height)
                            {
                    ...
                    && m_RangeEnabled[counter])
                    {
                        if ((m_MaxValue - m_MinValue) != 0)
                        {
                            ...
                            ggr.FillPie(new SolidBrush(m_RangeColor[counter]),
                                        new Rectangle(m_Center.X - m_RangeOuterRadius[counter],
                                                      m_Center.Y - m_RangeOuterRadius[counter],
                                                      2*m_RangeOuterRadius[counter], 2*m_RangeOuterRadius[counter]),
        ...
        public delegate void MouseValueSelectionHandler(float fClickedValue);
        [System.ComponentModel.Browsable(true),
        System.ComponentModel.Category("ucGauge"),
        System.ComponentModel.Description("Value returned via clicking the gauge--does NOT change value of gauge.")]
        public event MouseValueSelectionHandler OnMouseValueSelection;

        float? ValueFromMousePosition(int iX, int iY)
        {
            int iTrueX = iX - this.Center.X;
            int iTrueY = this.Center.Y - iY;

            double dblRadians = 0;
            if (iTrueY == 0)
            {
                if (iTrueX < 0)
                    dblRadians = 2 * Math.PI;
            }
            else if (iTrueX == 0)
            {
                if (iTrueY < 0)
                    dblRadians = 3 * Math.PI / 2;
                else
                    dblRadians = Math.PI / 2;
            }
            else
            {
                dblRadians = Math.Atan2(iTrueY, iTrueX);
            }

            double dblDegrees = Physics.Radians_to_Degrees(dblRadians);

            // The base arcs are in transformed coordinate system.
            double dblBaseArcStart = 360 - m_BaseArcStart;
            double dblRange = m_MaxValue - m_MinValue;

            double dblDegreesFromBaseArcStart = dblBaseArcStart - dblDegrees;
            if (dblDegreesFromBaseArcStart >= 360)
                dblDegreesFromBaseArcStart -= 360;

            // X: Math.Cos((m_BaseArcStart + (countValue * m_BaseArcSweep / (m_MaxValue - m_MinValue)))
            // Y: Math.Sin((m_BaseArcStart + (countValue * m_BaseArcSweep / (m_MaxValue - m_MinValue)))
            double value = dblDegreesFromBaseArcStart * dblRange / m_BaseArcSweep + m_MinValue;

            if (value < m_MinValue)
            {
                value = m_MinValue;
                //if (m_MinValue - value < 5)
                //    value = m_MinValue;
                //else
                //    return null;
            }
        ...
        protected override void OnMouseClick(MouseEventArgs e)
        {
            ...
            float? fValue = ValueFromMousePosition(e.X, e.Y);


            if (fValue.HasValue && OnMouseValueSelection != null)
            {
                OnMouseValueSelection((float)fValue.Value);
            }
        }
        ...
        public delegate void MouseDownValueSelectionHandler(float fClickedValue);
        [System.ComponentModel.Browsable(true),
        System.ComponentModel.Category("ucGauge"),
        System.ComponentModel.Description("Value returned via holding mouse button down on the gauge--does NOT change value of gauge.")]
        public event MouseDownValueSelectionHandler OnMouseDownValueSelection;
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            if (fValue.HasValue && OnMouseDownValueSelection != null)
            {
                OnMouseDownValueSelection((float)fValue.Value);
            }
        }
    }  // public partial class ucGauge : Control
